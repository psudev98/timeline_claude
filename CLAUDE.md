...
A private, two-person "romance timeline" web app (React 19 + Vite + Framer Motion + Supabase). One shared account per partner, no public signup. Views: Timeline, Calendar, Wall (polaroid grid), Letters, Someday (bucket list), Stats. Entry point is `index.html` → `src/main.tsx`. The frontend is a pure client-side SPA; the only server-side code is one Vercel serverless function (`api/roast.ts`) used to keep a third-party API key off the client — see "Serverless functions" below.
...
**Auth is unusual — read before touching it.** There's no email/password form. Each partner picks a named card, then answers one of three rotating trivia questions (first date / first kiss / anniversary) with a date picker. The trivia answer is compared entirely client-side against a `VITE_TRIVIA_*` env var and is **never** sent to Supabase. Only on a correct guess does the app call `signInWithPassword` behind the scenes using a separate hidden per-partner password (`VITE_PARTNER_*_AUTH_SECRET`) that the person never sees or types. A wrong guess never touches Supabase — it shows a line from the static `roastPool` immediately, then (best-effort) upgrades to an LLM-generated one via `fetchRoastLine()` → `api/roast.ts`. See README.md § 2 for the full mechanics.

**Env vars are `VITE_`-prefixed and get bundled into shipped client JS** — they're kept out of git via `.env.local`/`.gitignore`, but are not truly secret once deployed (anyone inspecting the deployed site's JS can read them). The `AUTH_SECRET` and `TRIVIA_*` values should still be treated as real passwords (long, random, unique per partner) since the auth-secret values literally *are* each account's Supabase Auth password. **`GROQ_API_KEY` is the one exception in this codebase — it must never be `VITE_`-prefixed**, since it's read server-side only by `api/roast.ts`; prefixing it would ship it to every visitor's browser.

## Serverless functions

`api/roast.ts` is a Vercel serverless function (Node runtime, typed via the `@vercel/node` devDependency) — the one piece of server-side code in this otherwise client-only app. It exists solely to call the Groq API (`llama-3.3-70b-versatile`, free tier) for a wrong-guess roast line without exposing `GROQ_API_KEY` to the client. Plain `npm run dev` (Vite) does not serve `/api/*` — the client's `fetchRoastLine()` in `main.tsx` treats a failed/404/timed-out call as "no upgrade available" and silently keeps the static `roastPool` line, so this degrades safely in local dev. To actually exercise the endpoint locally, run `vercel dev` instead. If more server-side logic is ever needed, follow this same pattern: a new file under `api/`, non-`VITE_`-prefixed env vars for anything secret, and a client-side caller that treats failure as "fall back to the client-only behavior," not as an error to surface to the user.
